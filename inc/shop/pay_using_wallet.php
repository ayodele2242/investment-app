...
$amt_to_pay = $_POST['mtotal'] + $shipping;
...
$wrow = mysqli_fetch_array($wquery);



if($wrow["amount_saved"] > $amt_to_pay){



//insert into transaction table
$iquery = mysqli_query($mysqli, "insert into transactions(product_id_array, payer_email, first_name, last_name, txn_id, receiver_email, payment_type, payment_status, address_street, 
address_city, address_state, address_zip, address_country,total_amt, 	coupon_code,shipping_fee)values('$myitems', '$email','$fname','$lname','$trxId','$email',
'Wallet Payment','Paid','$addr','$city','$state','$zip','$country','$tot_amt','$coupon_code','$shipping')");
...
$title = $mysqli->real_escape_string($arrayItem['title']);
...
$queryme = mysqli_query($mysqli,"insert into customer_order(
	...
	values('$email', '".$arrayItem['product_id']."', '$title', '".$arrayItem['image']."', '".$arrayItem['price']."', '".$arrayItem['quantity']."', '".$arrayItem['color']."', '".$arrayItem['size']."', '".$arrayItem['vendor']."','$amt','Wallet Payment','Paid','$trxId')") or die(mysqli_error($mysqli));
...
else // If $myList was not an array, then this block is executed. 
{
  ...
  echo "Error occured while processing cart items. Please try again. ";
    ...
    echo "You do not enough money in your wallet to purchase this item ";
}
