...
<body>
	...
	<header>
		...
		<div id="header">
			<div class="container">
				<div class="pull-left mobile-icon mobile-top">
					<!-- Logo -->
					<div class="header-logo mlogo-hide">
						<a class="logo" href="#">
							<?php
							if(!empty($set['logo'])){
								echo '<img src="'.$set['installUrl'].'assets/logo/'.$set['logo'].'" alt="">';
							}else{
								?>
								<img src="<?php echo FRONT_IMAGES;?>cart.png" alt="">
						<?php
							}
					...
					<!-- Search -->
					<div class="header-search mobile-search-header">
						<form method="get" action="search" class="search-header">
							<input class="input search-input imselect" name="keyword" type="text" placeholder="Enter your keyword">
							<select class="input search-categories mobile-hidemselect imselect" name="cat_id">
							...
							<button class="search-btn col-blue"><i class="fa fa-search"></i></button>
						</form>
					</div>
					<!-- /Search -->
				</div>
							...
							foreach ($parent_cats as $parents ) {
								...
								if ($child_cats) {
									...
									<li class="dropdown side-dropdown">
											<a class="dropdown-toggle" data-toggle="dropdown" aria-expanded="true" style="font-weight: bolder;">
													...
													<!--<div class="col-md-4 col-lg-4">-->
														<ul class="list-links row">
															...
															foreach ($child_cats as $children) {
																$ichild_cats = $category->getChildByParentId($children->id);
																?>
																<li class="col-lg-6 col-md-6">
																	<a href="category?child_id=<?php echo $children->id; ?>" class="bolder">
																		<?php echo stripcslashes($children->title); ?>
																	</a>
																	<ul>
																		<?php foreach ($ichild_cats as $ichildren) { ?>
																		<li class="firstLetter">
																			<a href="category?child_id=<?php echo $children->id; ?>">
																		<?php echo stripcslashes($ichildren->title); ?>
																	     </a>
									...
									?>
									<li><a href="category?cid=<?php echo $parents->id; ?>" style="font-weight: bolder;"> <?php echo stripcslashes($parents->title); ?></a></li>
									<?php 
								}
							}
						}
