...
if(!empty($_POST['name']) && !empty($_POST['duration']) && !empty($_POST['amount'])){
	...
	$sql2 = "SELECT * FROM saving_packages WHERE category = '$name' and duration = '$duration'";
    $result = mysqli_query($mysqli,$sql2) or die($mysqli->error);
    $count = mysqli_num_rows($result);
    if ($count < 1) {
    	$sql = "INSERT INTO saving_packages(category,duration,amount,details)values('$name','$duration','$amt','$descr')";
