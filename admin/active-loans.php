...
$status = FALSE;
if ( authorize($_SESSION["access"]["LOAN PRODUCTS"]["ACTIVE LOANS"]["create"]) || 
authorize($_SESSION["access"]["LOAN PRODUCTS"]["ACTIVE LOANS"]["edit"]) || 
authorize($_SESSION["access"]["LOAN PRODUCTS"]["ACTIVE LOANS"]["view"]) || 
authorize($_SESSION["access"]["LOAN PRODUCTS"]["ACTIVE LOANS"]["delete"]) ) {
 $status = TRUE;
}
          ...
          <div class="container">
            <div class="row">
              <div class="col s10 m6 l6">
                <h4 class="mt-0 mb-0 text-white" ><i class="material-icons">people</i> Active Loans</h4>
              </div>
    ...
    <div class="row">
     ...
     <table id="tableTrigger" class="table_view ">
       ...
       foreach($invest as $rows){
        ...
        <td>
           <?php if (authorize($_SESSION["access"]["LOAN PRODUCTS"]["ACTIVE LOANS"]["edit"])) { ?>
          <a href="#loan-modal" data-id="<?php echo $rows['aloan_id']; ?>" id="<?php echo $rows['aloan_id']; ?>" class="btn btn-floating waves-effect waves-light green z-depth-3 btn-small modal-trigger loanmodal" type="button" title="Edit"><i class="material-icons left">search</i></a>
        <?php } ?>
        </td>  
        ...
        <td><?php echo $rows['payment_schedule']; ?></td>
...
$(document).ready(function(){
    $(".loanmodal").click(function() {
     ...
     $.ajax({
          url: '../inc/loans/getLoansDetails.php',
          type: 'POST',
          data: 'uid='+pid,
          dataType: 'html'
     })
