...
$status = FALSE;
if ( authorize($_SESSION["access"]["PENDING WITHDRAWAL"]["PENDING WITHDRAWAL"]["create"]) || 
authorize($_SESSION["access"]["PENDING WITHDRAWAL"]["PENDING WITHDRAWAL"]["edit"]) || 
authorize($_SESSION["access"]["PENDING WITHDRAWAL"]["PENDING WITHDRAWAL"]["view"]) || 
authorize($_SESSION["access"]["PENDING WITHDRAWAL"]["PENDING WITHDRAWAL"]["delete"]) ) {
 $status = TRUE;
}
          ...
          <div class="container">
            <div class="row">
              <div class="col s10 m6 l6">
                <h4 class="mt-0 mb-0 text-white" ><i class="material-icons">people</i> Awaiting Withdrawal</h4>
              </div>
    ...
    <div class="row">
     ...
     <table class="table_view">
       ...
       foreach($row as $rows){
        ...
        <td><?php echo $rows['widrawal_type']; ?></td>
        ...
        <td class="tbtn"> 
          <!--<button id="<?php //echo  $rows['u_rolecode']; ?>" class="btn btn-floating uwithdrawal btn-small waves-effect waves-light orange z-depth-3"  ><i class="material-icons left">supervisor_account</i></button>-->
           <?php 
           if (authorize($_SESSION["access"]["PENDING WITHDRAWAL"]["PENDING WITHDRAWAL"]["edit"])) {
...
//Get PENDING WITHDRAWAL's details and update data
...
$(document).ready(function(){
    ...
    var serializedData = $("#paymentForm").serialize();
          ...
          $.ajax({
            ...
            if(datas.success == true){
                $(".roe-ibody").hide();
              	$("#resp").html('<div class="card card-success pr-3">Transfer has been queued.<br/>Enter OTP code enter here...</div>').show();
...
$(document).ready(function(){
    ...
    var serializedData = $("#otpForm").serialize();
          ...
          $.ajax({
            type : 'POST',
            url  : '../inc/payment/cashoutOtp.php',
            data : serializedData,
            success: function (data) {
            ...
            if(datas.success == true){
               ...
               setTimeout(' window.location.href = "pending_withdrawal"; ',10000);
