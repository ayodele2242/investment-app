...
public function getAllInfo($is_die = false){
   ...
   try{
         $this->fields('customer_address.*, customer_login.email as email_address, customer_login.first_name, customer_login.last_name');
         $this->join(' LEFT JOIN customer_login ON customer_address.uid = customer_login.id');
      $this->groupBy(' customer_address.id');
         $this->orderBy(' customer_address.id DESC');
         return $this->getData($is_die);
   }catch(Exception $e){
      return false;
   }
