...
class Customer_order extends Database
{
   ...
   public function getCustomerOrder($lid, $is_die = false){
    try {
        ...
        $this->join(' LEFT JOIN customer_info ON customer_order.c_email = customer_login.email LEFT JOIN customer_login on customer_address.uid = customer_login.id ');
